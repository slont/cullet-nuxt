<template>
  <div id="user-theme-item" @scroll="onScrollContainer">
    <nuxt-link :to="`/u/${$route.params.userId}/${themeId}`" tag="div"
          ...
          <div class="cullet-info">
            <div class="updated-at text-color-weak is-size-7 has-text-right" v-if="item.updatedAt">
              ...
              <span class="is-size-8" v-if="1 < $moment(item.updatedAt).diff($moment(item.createdAt), 'hours')">
                ({{ $moment(item.updatedAt).format('YYYY/MM/DD HH:mm') }} 更新)
              </span>
            </div>
...
<script>
  ...
  export default {
    ...
    created() {
      if (process.browser) {
        window.addEventListener('scroll', this.onScrollContainer);
      }
    },
    destroyed () {
      if (process.browser) {
        window.removeEventListener('scroll', this.onScrollContainer);
      }
    },
    mounted() {
      this.init()
    ...
    methods: {
      async init() {
        new ItemModel(this.themeId).findOne(this.itemId).then(res => {
          ...
          this.$nextTick(() => {
            if (80 > this.$el.clientHeight - window.innerHeight) {
      ...
      onScrollContainer() {
        ...
        if (80 > this.$el.clientHeight - window.innerHeight - scrollTop) {
          this.activePagination = true
        } else {
